 ...
 * Nessuna dipendenza da immagini: funziona in locale e in produzione.
 */
...
export function buildEmailHtml({ title, body, footerNote }: EmailOptions): string {
  return `<!DOCTYPE html>
<html lang="it">
<head>
  <meta charset="UTF-8" />
          ...
          <tr>
            <td style="background:${BRAND_GREEN};padding:32px 40px;text-align:center;">
              <p style="margin:0;color:#ffffff;font-size:26px;font-weight:700;
                         letter-spacing:1px;text-transform:uppercase;">Vinifera</p>
              <p style="margin:6px 0 0;color:#a7c4b8;font-size:11px;
                         letter-spacing:2px;text-transform:uppercase;">Studio Enologico</p>
            </td>
          </tr>
          <!-- Body -->
          <tr>
          ...
          <tr>
            <td style="padding:20px 40px 32px;border-top:1px solid #e5e7eb;">
              <p style="margin:0;font-size:12px;color:#9ca3af;text-align:center;">
                ${footerNote ? escHtml(footerNote) + "<br/>" : ""}
                Questa è una notifica automatica di Vinifera CRM.
              </p>
            </td>
          </tr>
        </table>
