...
const ServiceClientSchema = z.object({
  ...
  benefits_it: z.array(z.object({ value: z.string() })),
  benefits_en: z.array(z.object({ value: z.string() })),
  faq_it: z.array(z.object({ q: z.string(), a: z.string() })),
...
function toFormValues(doc?: ServiceDoc): FormInput {
  return {
    ...
    benefits_it: (doc?.benefits?.it ?? []).map((v) => ({ value: v })),
...
function toServerPayload(values: FormInput) {
  ...
  return {
    ...
    benefits: {
      it: values.benefits_it.map((b) => b.value).filter((v) => v.trim() !== ""),
      en: values.benefits_en.map((b) => b.value).filter((v) => v.trim() !== ""),
    },
    faq: {
      it: values.faq_it.filter((f) => f.q.trim() !== "" || f.a.trim() !== ""),
