...
- [ ] CRM (`vinifera-crm`) avviato su `http://localhost:3000` con `NEXT_PUBLIC_DEV_BYPASS_AUTH=true`
- [ ] Sito (`vinifera-site`) avviato su porta diversa (es. 3001)
- [ ] `stripe listen --forward-to localhost:3000/api/stripe/webhook` attivo in un terminale
- [ ] `STRIPE_WEBHOOK_SECRET` nel `.env.local` del CRM corrisponde al secret stampato da Stripe CLI
- [ ] Almeno un evento creato nel CRM (vedi §1)
...
### Creazione
- [ ] Creo un evento **a pagamento**: titolo IT+EN, data futura, location, capienza 10, prezzo €30,00
- [ ] Creo un evento **gratuito**: attivo lo switch "Evento gratuito", inserisco `maxSeatsPerOrder: 4`
- [ ] **Slug duplicato rifiutato**: provo a creare un secondo evento con lo stesso slug → errore inline
- [ ] Evento creato sempre in stato **Bozza** (badge grigio outline)

### Pubblicazione e stati
- [ ] "Pubblica" → stato passa a Published, badge "Prenotabile" (se data ok)
- [ ] "Nascondi" → torna a Bozza
- [ ] Il badge sul sito si aggiorna entro ~5 minuti (o subito se revalidation funziona)

### Riduzione capienza con overbooking
- [ ] Con 3 ordini paid (6 posti) riduco la capienza a 4 → salva, appare alert arancione nell'interfaccia
...
### Upload immagine
- [ ] Carico un'immagine dalla scheda evento → URL viene popolato, anteprima visibile
- [ ] "Rimuovi" cancella l'immagine da Storage e azzera il campo

### Archivio
- [ ] "Archivia" → evento sparisce dalla lista principale
- [ ] Toggle "Mostra archiviati" → appare
- [ ] "Ripristina" → torna visibile
...
### Lista `/it/eventi`
- [ ] Lista eventi mostra eventi futuri nella sezione "Prossimi eventi"
- [ ] Badge `bookable` → sfondo verde primario, testo dorato
- [ ] Badge `upcoming` → sfondo dorato, testo verde
- [ ] Badge `sold_out` → sfondo rosso vino, testo bianco
- [ ] Badge `past` → sfondo grigio muted
- [ ] Evento gratuito mostra **"Gratuito"** (mai "0,00 €")

### Dettaglio `/it/eventi/[slug]`
- [ ] Hero, titolo, data, luogo, prezzo renderizzati correttamente
- [ ] Prezzo scontato → prezzo pieno barrato + prezzo scontato
- [ ] Evento gratuito → "Gratuito" al posto del prezzo (mai cifre)
- [ ] CTA "Prenota" visibile solo se `bookable`
- [ ] CTA "Prenota" assente se `upcoming`, `sold_out`, `past`
- [ ] JSON-LD `Event` presente nella sorgente (Ctrl+U → cerca `application/ld+json`)

### Banner home
- [ ] Evento `featured: true` e `bookable` → banner slide-in visibile in homepage
- [ ] Click sulla X → banner scompare e non riappare nella stessa sessione del browser
- [ ] Riaprendo la tab o in sessione nuova → banner riappare
- [ ] Nessun evento featured/bookable → nessun banner (nessun elemento vuoto)

### Versione EN
- [ ] `/en/eventi` mostra testi in inglese (titoli, badge, CTA)
...
### Flusso completo
- [ ] Step 1: seleziono 2 posti — massimo rispettato (frecce e digitazione bloccate al max)
- [ ] Step 2: acquirente + 2 partecipanti + checkbox consenso storico (facoltativa)
- [ ] Step 3: profilo fatturazione **Privato** con CF valido (es. `RSSMRA85T10A562S`)
- [ ] Step 3: profilo fatturazione **Azienda** — compilo P.IVA + SDI o PEC (almeno uno)
- [ ] Step 4: Payment Element visibile, countdown mm:ss visibile e decrescente
- [ ] Carta test: `4242 4242 4242 4242`, scadenza `12/29`, CVC `123`
- [ ] Pagamento → redirect a `/esito?order_id=…`
- [ ] Pagina esito mostra "Prenotazione confermata!", numero ordine, posti, totale
- [ ] Email conferma ricevuta (controlla casella o log Resend)
- [ ] Nel terminale Stripe CLI: evento `payment_intent.succeeded` ricevuto

### Verifica nel CRM
- [ ] CRM → `/events/[id]/orders` → ordine visibile con stato "Pagato" (badge verde)
- [ ] Drawer ordine: acquirente, partecipanti, profilo fatturazione completo
- [ ] Bottone "Copia dati fatturazione" → copia testo strutturato negli appunti
- [ ] `seatsSold` incrementato correttamente sull'evento

### Resume hold
- [ ] Inizio checkout, arrivo al Payment Element ma NON pago
- [ ] Refresh della pagina → ripeto i primi step con **stessa email** sullo **stesso evento**
- [ ] → Non crea un secondo hold: riusa il PaymentIntent esistente (campo `publishable_hint: "resume"`)
- [ ] Countdown ricomincia dal tempo residuo, non da 15 minuti

### Pagamento fallito
- [ ] Carta declinata: `4000 0000 0000 0002`
- [ ] Messaggio di errore visibile, countdown continua
- [ ] L'utente può riprovare con un'altra carta

### Idempotenza webhook (smoke test — la garanzia è già coperta da test unitari)
- [ ] Dal terminale Stripe CLI copia l'ID dell'evento `payment_intent.succeeded` (es. `evt_xxx`)
- [ ] Esegui `stripe events resend evt_xxx`
- [ ] Nel CRM: `seatsSold` NON cambia, nessuna email duplicata

---

## §4 — Checkout: evento gratuito (M3)

- [ ] Step 1: seleziono posti ≤ `maxSeatsPerOrder` → ok; > max → valore clampato istantaneamente
- [ ] Step 2: acquirente + partecipanti (nessun step fatturazione)
- [ ] Submit → redirect immediato a `/esito` (nessun Payment Element, nessun countdown)
- [ ] Pagina esito: "Prenotazione confermata!", totale mostra "Gratuito"
- [ ] Email conferma ricevuta (senza menzione di ricevuta Stripe)
- [ ] CRM: ordine `paid` con `totalCents: 0`, `paymentIntentId: null`, `billing: null`
- [ ] `seatsSold` incrementato, `seatsHeld` MAI toccato
- [ ] Nessun PaymentIntent nella dashboard Stripe test
...
- [ ] Avvio checkout a pagamento, arrivo al Payment Element, **aspetto 1 minuto + ~90 secondi**
- [ ] Countdown mostra 00:00 → appare schermata "Prenotazione scaduta"
- [ ] La Cloud Function (o emulatore) cancella il PaymentIntent
- [ ] CRM: ordine passa a `expired`, `seatsHeld` decrementato
- [ ] CTA "Ricomincia" porta alla pagina evento
...
- [ ] CRM → ordine `paid` a pagamento → Drawer → "Rimborsa ordine"
- [ ] Stripe CLI mostra `charge.refunded`
- [ ] Ordine passa a `refunded`, `seatsSold` decrementato
- [ ] Badge event si aggiorna (posti tornano disponibili)

### Annullamento ordine gratuito
- [ ] CRM → ordine `paid` gratuito → Drawer → "Annulla prenotazione"
- [ ] Ordine passa a `cancelled`, `seatsSold` decrementato immediatamente (senza webhook)
- [ ] Nessuna chiamata Stripe (verificabile: nessun evento nella CLI)

### Cancellazione evento con rimborsi (misto)
- [ ] Creo un evento con 1 ordine a pagamento + 1 gratuito
- [ ] CRM → dettaglio evento → "Cancella evento con rimborsi"
- [ ] Dialog mostra dettagli → confermo
- [ ] Evento sparisce dal sito
- [ ] Ordine a pagamento → Stripe CLI mostra `charge.refunded` → stato `refunded`
- [ ] Ordine gratuito → stato `cancelled` immediato, senza Stripe
- [ ] Email corrette inviate a entrambi gli acquirenti (uno riceve menzione rimborso, l'altro no)

### "Notifica tutti"
- [ ] CRM → dettaglio evento con almeno 1 ordine paid → sezione "Notifica tutti"
- [ ] Scrivo oggetto + testo → "Invia a tutti"
- [ ] Email ricevuta dall'acquirente (e da eventuali altri, deduplicati per email)
...
### Iscrizione
- [ ] Sito → `/it/eventi` → form "Tienimi aggiornato"
- [ ] Inserisco email valida + spunto consenso → "Controlla la tua email per confermare"
- [ ] Ricevo email con link di conferma (o vedo il log in Resend)
- [ ] Click sul link → `/it/eventi/conferma-iscrizione?token=…` → "Iscrizione confermata!"
- [ ] CRM → `/events/subscribers` → iscritto visibile con stato "Attivo"
