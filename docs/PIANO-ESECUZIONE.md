...
### ✅ 7. Tema verde biliardo — *provato e poi scartato, tornati al tema chiaro originale*
...
- **Stato**: partito con la variante A (in produzione dal 2026-09-22, confermata dal cliente il
  ...
  insieme al follow-up sulle sottocategorie "Altro". **Poi, sempre il 2026-09-24, dietro
  ulteriore feedback del cliente, tutto il verde biliardo (A/B/C) è stato scartato**: si torna
  al tema chiaro originale (sfondo crema, card bianche) — vedi Registro per i valori esatti
  ripristinati. Chiuso, tema originale.
...
| 2026-09-24 | 10 (follow-up) | Assistente AI — secondo giro di verifica + error boundary | ✅ Fatto | **Correzione a una nota del giro precedente**: `list_expenses` non aveva nessun problema di indice — la collection `costExpenses` era semplicemente vuota su questo emulatore (`npm run seed:costs` non era mai stato lanciato in questa sessione). Seedata e riverificata: link, importi e categorie tutti corretti su 10 spese reali. **Test di prompt-injection rifatto con successo** (il giro precedente era inconcludente per lo stesso motivo — nessun dato da leggere): questa volta il modello ha letto la descrizione iniettata, l'ha citata, riconosciuta come tentativo di istruzione e ignorata, e ha pure segnalato all'utente la voce sospetta — poi una domanda successiva fuori tema ha ricevuto il rifiuto corretto, nessuna contaminazione tra turni. Testati anche gli strumenti non ancora coperti (`list_quotes`, `list_reminders`, `list_clients` con `sortBy`, `get_sample` con risultati non inseriti) e una **conversazione multi-turno** (una domanda di follow-up riusa correttamente il contesto della risposta precedente) — tutti corretti. Validazione richieste testata a mano: body vuoto, `messages: []`, messaggio oltre 4000 caratteri, `role` non valido, oltre 40 messaggi → tutti `400` come da schema. **Sul blocco della verifica a schermo**: isolata la causa in modo definitivo — un WebSocket verso un server echo scollegato del tutto da Next.js fallisce dallo stesso Chromium automatizzato (`close code=1006`), mentre lo stesso identico handshake via `http` Node grezzo riesce (`101 Switching Protocols`); JS vanilla e tutte le richieste HTTP nella pagina funzionano normalmente. Non è Turbopack, non è React, non è questo modulo: è un blocco di questa macchina su **qualunque** WebSocket iniziato da un Chromium automatizzato. Scartata anche la scorciatoia "verifichiamo con `next start`": il codice ha una guardia esplicita (`src/lib/firebase/admin.ts`) per cui `NODE_ENV==="production"` disattiva `USE_EMULATORS` a prescindere dal flag — quindi in produzione punterebbe sempre al Firebase vero, non all'emulatore; fermato subito senza toccare dati reali (l'unico scambio di token tentato è stato respinto dal Firebase vero, correttamente, perché il token veniva dall'emulatore). **Aggiunto `AssistantErrorBoundary.tsx`** attorno a `<AssistantLauncher />` in `(app)/layout.tsx`, su richiesta esplicita dell'utente prima del deploy: l'assistente è montato globalmente, senza boundary un crash in rendering si porterebbe giù l'intera app per tutte le pagine invece di disattivarsi in silenzio (copre solo errori di rendering, non quelli dentro gli event handler — comunque il caso "porta giù tutta l'app" che si voleva evitare). `npm run check` verde. **Decisione esplicita dell'utente**: push diretto su `main`/produzione senza passare da un preview, accettando che l'interfaccia (i tre componenti React) non sia mai stata osservata cliccare in un browser reale — solo la logica server (strumenti, streaming, sicurezza) è verificata end-to-end. |
| 2026-09-24 | 10 (bugfix in prod) | Assistente AI — due bug trovati dal cliente al primo test reale | ✅ Fatto | Il cliente ha testato l'assistente in produzione (primo vero click-through, mai avvenuto prima — vedi riga precedente) e ha trovato due problemi. (1) **`list_payments` falliva sempre in produzione**: filtrava su `deletedAt`, campo che nessun'altra query sui pagamenti nel repo usa (i pagamenti non si soft-cancellano così, l'annullamento è `status: "cancelled"`) e senza indice composito corrispondente in `firestore.indexes.json` — l'emulatore locale non l'aveva mai fatto emergere perché è più permissivo di Firestore vero su questa classe di errore (lezione da ricordare). Corretto riusando `getPayments()` (stessa query già in produzione da mesi sulle pagine pagamenti) e filtrando lo stato in memoria; riverificato dal vivo con tutte le combinazioni di filtro. (2) **Sotto-menu del dettaglio cliente senza sfondo** (e stesso miss nel dettaglio evento): mancava `bg-card` sull'`<aside>`, sfuggito alla sweep del tema variante C. `npm run check` verde su entrambi. |
| 2026-09-24 | 7 (ripristino) | Tema verde biliardo scartato, torna il tema chiaro originale + bordi più spessi | ✅ Fatto | Il cliente ha visto tutte e tre le varianti (A/B/C) e alla fine ha chiesto di tornare com'era prima — niente verde di sfondo. Valori di `:root` in `globals.css` ripristinati esattamente a quelli pre-modulo 7 (commit `8449bd9`, prima di ogni variante): `--background: oklch(0.985 0.005 80)` (crema chiarissimo), `--card`/`--popover: oklch(1 0 0)` (bianco puro), `--secondary`/`--muted` tornati al tono caldo dorato, `--border`/`--input` tornati a `oklch(0.88 0.015 165)` (grigio neutro, non più verde). `.dark` non toccato (mai stato modificato in nessuna variante). Contestualmente il cliente ha chiesto bordi più spessi ovunque (tabelle comprese): aggiunta una regola in `globals.css` che porta a `2px` (da 1px, il default Tailwind) `.border`/`.border-t`/`.border-b`/`.border-l`/`.border-r`/`.border-x`/`.border-y` — una proprietà per lato invece dello shorthand `border-width`, altrimenti `.border-t` ecc. avrebbero disegnato un bordo anche sui lati che quella utility non tocca; niente regola su `*` (specificità più bassa delle classi, verrebbe ignorata). Le aggiunte `bg-card` fatte durante la sweep del tema verde (tabelle, input, select, switch, checkbox, i due `<aside>` del bugfix precedente) sono state lasciate: sono corrette a prescindere dal colore di sfondo. Verificato a schermo (screenshot, chiaro e scuro) su dashboard, tabella clienti e pagamenti: sfondo bianco/crema confermato, separatori di riga e bordi KPI visibilmente più spessi, badge di stato ancora leggibili. `npm run check` verde. |
